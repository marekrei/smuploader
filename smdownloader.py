from smugmug import SmugMug
import argparse, sys, os, hashlib

parser = argparse.ArgumentParser(description='Download images from SmugMug.')
parser.add_argument('-a', '--album', dest='album', metavar='ALBUM_NAME', type=str, help='album to download')
parser.add_argument('--getall', dest='getall', action='store_true', default=False, help='download all albums')
parser.add_argument('-p', '--path', dest='path', metavar='PATH', type=str, help='path where the albums are downloaded')
parser.add_argument('-r', '--resume', dest='resume', action='store_true', default=False, help='if album already exists, add photos in there. default: false')
parser.add_argument('-v', '--verbose', dest='verbose', action='store_true', default=False, help='verbose output')
args = parser.parse_args()

album_name = args.album
getall = args.getall
path = args.path
resume = args.resume
verbose = args.verbose
retries = 5

if album_name == None and getall == False:
	print "Error: No albums specified"
	sys.exit(1)

if path == None:
	print "Error: Path is not specified"
	sys.exit(1)

smugmug = SmugMug(verbose)

album_names = []
if album_name != None:
	album_names.append(album_name)
elif getall == True:
	albums = smugmug.get_album_names()
	for album in albums:
		album_names.append(album)

for album_name in album_names:
	print "#### Processing album: " + album_name

	# Creating the album
	album_path = os.path.join(path, album_name)
	if os.path.exists(album_path):
		if resume == False:
			print "Error: Local album \'" + album_name + "\' already exists and resuming is not enabled."
			sys.exit(1)
		else:
			print "Local album already exists. Adding images."
	else:
		print "Creating new local album."
		os.makedirs(album_path)

	album_id = smugmug.get_album_id(album_name)
	if album_id == None:
		print "Error: Album does not exist at SmugMug. Skipping."
		continue
	images_info = smugmug.get_album_images_info(album_id)
	count = 0
	total = len(images_info)
	for image_info in images_info:
		count += 1
		image_url = image_info['original_url']
		image_name = image_info['name']
		image_path = os.path.join(album_path, image_name)
		sys.stdout.write('Downloading ' + album_name + '/' + image_name + ' [' + str(count) + '/' + str(total) + ']... ')
		sys.stdout.flush()
		if verbose == True:
			print ''

		if os.path.exists(image_path):
			if resume == True:
				print "Already exists. Skipping."
			else:
				print "Error: File already exists and resuming is not enables."
		else:
			smugmug.download_image(image_info = image_info, image_path = image_path)
			print "Done"

print "All done."

